import { Enclosure, getAnimalsByEnclosure, getSpeciesById, speciesEmojis } from "@/data/zooData";
...
const EnclosureCard = ({ enclosure, onClick }: EnclosureCardProps) => {
  const animals = getAnimalsByEnclosure(enclosure.id);
  const animalSpecies = animals.map(a => getSpeciesById(a.speciesId));
    ...
    >
      ...
      <div className="bg-card p-3 md:p-4">
        ...
        <p className="text-xs text-muted-foreground font-nunito mt-1">
          {animals.length} animal{animals.length !== 1 ? "s" : ""} • {enclosure.areaSqm} sqm
        </p>
      </div>
    </button>
  );
